...
import com.data.session16a.connection.ConnectionDB;
...
public class ResumeDAO {
    ...
    public void updateResume(Resume resume) throws SQLException {
        String query = "UPDATE resumes SET fullName = ?, email = ?, phoneNumber = ?, education = ?, experience = ?, skills = ? WHERE id = ?";
        try(Connection connection = ConnectionDB.openConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            preparedStatement.setString(1, resume.getFullName());
            preparedStatement.setString(2, resume.getEmail());
            ...
            preparedStatement.setString(5, resume.getExperience());
            preparedStatement.setString(6, resume.getSkills());
            preparedStatement.setLong(7, resume.getId());
            preparedStatement.executeUpdate();
        }
    }
